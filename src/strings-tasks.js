...
function removeFirstOccurrences(str, value) {
  ...
  const { length } = value;
  if (idx === -1) return str;
  if (idx === 0) return str.slice(length);
  return str.slice(0, idx).concat(str.slice(idx + length));
}
...
function encodeToRot13(str) {
  function ROT13(code) {
    ...
    return (code > 64 && code < 78) || (code > 96 && code < 110)
      ? code + 13
      : code - 13;
  }
  return str
    .split('')
    .map((c) => String.fromCharCode(ROT13(c.charCodeAt(0))))
    .join('');
}
...
function getCardId(value) {
  let id = 0;
  const mark = value.slice(value.length - 1);
  const card = value.slice(0, value.length - 1);
  if (mark === '♦') id += 13;
  else if (mark === '♥') {
    id += 26;
  } else if (mark === '♠') {
